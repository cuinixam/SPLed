/**
 * @docs
 * Module to interface with power button keyboard
 * @enddocs
 */
...
static KeyState currentState =
    INIT; /**< Current state of the debouncing state machine. */
...
/**
 * @docs
 * Debounces key presses and releases.
 *
 * This function utilizes a state machine to debounce key presses and releases.
 * It must be called periodically to process the debouncing.
 *
 * The behavior is controlled by two configuration parameters:
 *   - POWER_BUTTON_PRESS_DEBOUNCE
 *   - POWER_BUTTON_RELEASE_DEBOUNCE
 *
 * which determine how many consecutive calls with the key pressed/released
 * are required to acknowledge the state transition.
 * @enddocs
 */
